...
Wall::Wall(int type, int rot, GLuint ** textures, bool* isWall, float size, float x, float y):
    m_texID(type), m_rot(rot), m_textures(textures), m_isWall(isWall), m_size(size), m_xPos(x), m_yPos(y){
    float vboData[4*4*2 *2] = {   x, y,
                                x+size, y,
                                x+size, y+size,
                                x, y+size,
                                0, 1,
    ...
    glBindVertexArray(m_vaoID);
        ...
        glBindBuffer(GL_ARRAY_BUFFER, m_vboID);
            glBufferData(GL_ARRAY_BUFFER, 4*4*2 *2 *sizeof(float), vboData, GL_STATIC_DRAW);
            
            glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));
            glEnableVertexAttribArray(0);

            glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(4*2 * sizeof(float)));
    ...
    glBindVertexArray(m_vaoID);
        glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, (*m_textures)[m_texID]);
                glDrawArrays(GL_TRIANGLE_FAN, m_rot*8, 4);
