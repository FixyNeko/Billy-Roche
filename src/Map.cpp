...
Map::Map(int x, int y, std::vector<int> str, std::vector<int> strRot, bool * isWall, float cellSize,
    ...
    for(int j = 0; j < y; j++){
        for(int i = 0; i < x; i++){
            m_layout.push_back(new Wall(str[j*x + i], strRot[j*x + i], &m_textures, isWall, m_cellSize, i*m_cellSize, j*m_cellSize));
        }
    }
}
